...
def main(page: flet.Page):
    # Define "counter_value" variable as integer of value "0"
    counter_value: int = 0
    ...
    counter_value_ui_text = flet.Text(str(counter_value), size=50)
    ...
    def increment_counter_value(e):
        nonlocal counter_value
        counter_value += 1
        counter_value_ui_text.value = str(counter_value)
        counter_value_ui_text.update()

    # Set up the floating action button
    # - Creates a circular button with a plus icon
    # - Associates our "counter_value_ui_text" function as the click handler
    page.floating_action_button = flet.FloatingActionButton(
        icon=flet.Icons.ADD, on_click=increment_counter_value
    )

    # Add the "counter_value_ui_text" to the page
